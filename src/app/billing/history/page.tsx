...
export default function BillingHistoryPage() {
  ...
  return (
    <div className="container py-12 md:py-16 max-w-3xl">
      <h1 className="text-3xl font-bold mb-2">{t('billing.history', language) || 'Billing History'}</h1>
      <p className="text-muted mb-6">{t('billing.recentPayments', language) || 'Your recent payments'}</p>
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl p-6 bg-[var(--surface)] ring-1 ring-white/5">
          ...
          <div className="mt-4">
            <Link href="/pricing" className="px-4 py-2 rounded-md bg-primary text-black font-semibold hover:opacity-90 inline-block">{t('pricing.getStarted', language) || 'Get Started'}</Link>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          {rows.map((p) => (
            <div key={p.id} className="rounded-xl p-4 bg-[var(--surface)] ring-1 ring-white/5 flex items-center justify-between">
              <div>
                <div className="text-white font-semibold">{p.description || 'Subscription payment'}</div>
